...
def Fog_Event_(Parent):
    ...
    for drag in Fog["rewards"]:
        Dragon_Reward_Dict[drag['id']] = drag['reward_id']
    ...
    for grid in Fog['squares']:
        ...
        if grid['type'] == 'DRAGON_PIECE':
            grid_id = grid['reward_id']
            if not grid_id in Fog_Rewards['DRAGON_PIECE']:
                dragon_url = "https://dca-static-s1.socialpointgames.com/static/dragoncity/mobile/ui/dragons/HD/thumb_"+Parent.Dragon_Info[Dragon_Reward_Dict[grid_id]]['img_name_mobile']+'_3.png'
                ...
                Fog_Rewards['DRAGON_PIECE'][grid_id] = {"image":PIL.Image.fromarray(cv2.resize(array_temp,dsize=Icon_Sizing,interpolation=cv2.INTER_CUBIC)),"url":dragon_url}
    ...
    for Square in Fog['squares']:
        ...
        if Square['type'] == 'CHEST':
            if Square['type_id'] in Chests_of_Interest:
                if Chest_Dragon_Identifier != "":
                    Fog_Board_Chests_Visual.paste(Fog_Rewards[Square['type']][Chest_Dragon_Identifier],(x_placement+15,y_placement+15,x_placement+215,y_placement+215))
                    Fog_Board_Dragons_and_Chests_Visual.paste(Fog_Rewards[Square['type']][Chest_Dragon_Identifier],(x_placement+15,y_placement+15,x_placement+215,y_placement+215))
        ...
        if Square['type'] == 'DRAGON_PIECE':
            if Chest_Dragon_Identifier != "":
                ...
                Fog_Board_Dragons_Visual.paste(im=Fog_Rewards[Square['type']][Chest_Dragon_Identifier],box=(x_placement+15,y_placement+15,x_placement+215,y_placement+215))
                Fog_Board_Dragons_and_Chests_Visual.paste(Fog_Rewards[Square['type']][Chest_Dragon_Identifier],(x_placement+15,y_placement+15,x_placement+215,y_placement+215))
