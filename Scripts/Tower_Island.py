...
def Tower_Event_(Parent):
	...
	for x in Parent.data_view['tower_island']['squares']:
		...
		if x['type'] == 'SINGLE_REWARD':
			...
			if 'chest' in x['rewards_array'][0]:
				if x['rewards_array'][0]['chest'] in Parent.Chests_Desired:
					include_check = 1
					Chest_Name = Parent.Chest_Naming(Parent.Chest_Ids[x['rewards_array'][0]['chest']])
			if 'b' in x['rewards_array'][0]:
				include_check = 1
			...
			if include_check == 1:
				...
				Key_filePath.write(f'C{str(Chest_Counter)} - {Parent.Chest_Name}\n')
				Chest_Counter+=1
	...
	Tower_Image.save(Parent.Event_fP+'Tower_Map_'+str(int(Event_Start_Date[8:10]))+' '+Event_Start_Date[4:7]+' '+Event_Start_Date[-4:]+'.jpg')
